...
export function AgencyAnalysisDrawer({
  analysis,
  open,
  onOpenChange
}: AgencyAnalysisDrawerProps) {
  ...
  const {
    data: linkedEntities = []
  } = useLinkedEntitiesForAnalysis(analysis?.id);

  // Fetch ticket count
  const {
    data: ticketCount = 0
  } = useTicketCountByAnalysis(analysis?.id);
  if (!analysis) return null;
  const acceptanceStatus = getAcceptanceStatus(analysis);
  const acceptanceUrl = analysis.acceptance_token ? `${window.location.origin}/aceite/${analysis.acceptance_token}` : '';
  ...
  const handleRequestNewLink = async () => {
    ...
    try {
      ...
      const {
        data: masters
      } = await supabase.from('user_roles').select('user_id').eq('role', 'master');
      ...
      queryClient.invalidateQueries({
        queryKey: ['ticket-count-by-analysis']
      });
      ...
      <SheetContent className="w-full sm:max-w-2xl p-0">
        <SheetHeader className="p-6 pb-4 border-b">
          <div className="flex items-start justify-between">
            ...
            <Badge variant="secondary" className={`status-badge ${statusConfig[analysis.status].class}`}>
          ...
          <ScrollArea className="flex-1">
            ...
            <TabsContent value="resumo" className="m-0 p-6">
              <div className="space-y-6">
                ...
                {analysis.status === 'aguardando_pagamento' && analysis.acceptance_token && <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
                    ...
                    <div className="flex gap-2">
                      <Input value={acceptanceUrl} readOnly className="text-xs font-mono" />
                      <Button size="sm" variant="outline" onClick={handleCopyLink} disabled={acceptanceStatus?.status === 'expired'}>
                        <Copy className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="outline" onClick={handleRequestNewLink} disabled={requestingLink} title="Solicitar novo link">
                        {requestingLink ? <Loader2 className="h-4 w-4 animate-spin" /> : <MessageSquare className="h-4 w-4" />}
                      </Button>
                    </div>
                    
                    {acceptanceStatus?.status === 'expired' && <p className="text-xs text-muted-foreground mt-2">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid gap-1">
                    ...
                    <InfoRow label="Endereço" value={`${analysis.imovel_endereco}${analysis.imovel_numero ? `, ${analysis.imovel_numero}` : ''}${analysis.imovel_complemento ? ` - ${analysis.imovel_complemento}` : ''}`} icon={MapPin} />
            ...
            <TabsContent value="documentos" className="m-0 p-6">
              <DocumentSection analysisId={analysis.id} identityPhotoPath={analysis.identity_photo_path} tenantName={analysis.inquilino_nome} />
            </TabsContent>

            {/* Chamados Tab */}
            <TabsContent value="chat" className="m-0 h-full">
              <AnalysisTicketSection analysisId={analysis.id} agencyId={analysis.agency_id} tenantName={analysis.inquilino_nome} isAgencyPortal={true} />
            </TabsContent>
          </ScrollArea>
        </Tabs>
      </SheetContent>
