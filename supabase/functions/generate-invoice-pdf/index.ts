...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────────
    // AUTH GUARD — TR-AUTH-002 fix
    // Antes: qualquer um com invoiceId UUID gerava PDF cross-tenant.
    // Agora: caller precisa estar autenticado + ser master OU pertencer
    // à agency dona da invoice.
    // ─────────────────────────────────────────────────────────────────
    ...
    // Tenant guard — agency_user só vê PDF da própria agency
    ...
    console.log("[generate-invoice-pdf] caller=", caller.id, "isMaster=", isMaster);
    ...
    // Tenant guard: agency_user só vê invoices das próprias agencies
    if (allowedAgencyIds) {
