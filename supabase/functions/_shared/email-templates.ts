// Templates de e-mail com branding GarantFácil
...
export function generateEmailWrapper(content: string, preheader?: string, heroImage?: string): string {
  const heroSection = heroImage ? `
          ...
          <tr>
            <td style="padding:0;">
              <img src="${heroImage}" alt="GarantFácil" style="width:100%;max-width:600px;height:180px;object-fit:cover;object-position:center;display:block;" />
            </td>
          </tr>
  ` : '';
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 6px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="background-color:${TRIDOTS_BLUE};padding:30px 40px;text-align:center;">
              ...
              <!--[if mso]>
              <span style="font-family:Arial,sans-serif;font-size:24px;font-weight:bold;color:#ffffff;">GARANTFÁCIL</span>
              <![endif]-->
            </td>
          </tr>
          ${heroSection}
...
export function acceptanceDigitalTemplate(data: {
  ...
  const content = `
    ...
    <p style="margin:0 0 20px 0;font-size:16px;color:#374151;line-height:1.6;">
      Sua análise de garantia locatícia foi aprovada pela <strong>GarantFácil</strong>! 
    ...
    <p style="margin:20px 0 0 0;font-size:14px;color:#6b7280;line-height:1.6;">
      Ao concluir o aceite digital, você confirma a contratação da garantia locatícia junto à <strong>GarantFácil</strong> e 
      autoriza a cobrança conforme os termos apresentados.
    </p>
  `;

  return {
    subject: `${data.tenantName}, seu aceite digital está disponível - GarantFácil`,
...
export function renewalReminderTemplate(data: {
  ...
  const content = `
    ...
    <p style="margin:20px 0 0 0;font-size:14px;color:#6b7280;line-height:1.6;">
      ...
      e continuar protegido pela <strong>GarantFácil</strong>.
    </p>
  `;

  return {
    subject: `${data.tenantName}, seu contrato expira em ${data.daysRemaining} dias - GarantFácil`,
...
export function paymentConfirmationTemplate(data: {
  ...
  const content = `
    ...
    <p style="margin:20px 0 0 0;font-size:14px;color:#6b7280;line-height:1.6;">
      Aguarde a ativação do seu contrato pela <strong>GarantFácil</strong>. Você receberá um e-mail quando estiver tudo pronto!
    </p>
  `;

  return {
    subject: `${data.tenantName}, seu pagamento foi confirmado - GarantFácil`,
...
export function contractActivatedTenantTemplate(data: {
  ...
  const content = `
    <h1 style="margin:0 0 20px 0;font-size:24px;color:${TRIDOTS_BLUE};font-weight:600;">
      Bem-vindo à GarantFácil! 🏠
    </h1>
    <p style="margin:0 0 20px 0;font-size:16px;color:#374151;line-height:1.6;">
      Olá, <strong>${data.tenantName}</strong>!
    </p>
    <p style="margin:0 0 20px 0;font-size:16px;color:#374151;line-height:1.6;">
      Seu contrato de garantia locatícia foi ativado com sucesso! Você está protegido pela GarantFácil.
    </p>
    ...
    <p style="margin:20px 0 0 0;font-size:14px;color:#6b7280;line-height:1.6;">
      Mantenha seus pagamentos em dia para continuar protegido. Em caso de dúvidas, entre em contato pelo portal da <strong>GarantFácil</strong>.
    </p>
  `;

  return {
    subject: `${data.tenantName}, seu contrato está ativo - GarantFácil`,
...
export function contractActivatedAgencyTemplate(data: {
  ...
  const content = `
    ...
    <p style="margin:0 0 20px 0;font-size:16px;color:#374151;line-height:1.6;">
      O contrato abaixo foi ativado com sucesso e está sob cobertura da GarantFácil.
    </p>
  ...
  return {
    subject: `${data.tenantName} - Contrato Ativado - GarantFácil`,
...
export function agencyActivationTemplate(data: {
  ...
  return {
    subject: `${data.responsibleName}, seu cadastro foi aprovado - GarantFácil`,
    html: generateEmailWrapper(content, 'Seu cadastro foi aprovado! Acesse o portal.', IMAGES.handshake)
  };
}

// Template: Nova Imobiliária Pendente (GarantFácil)
export function newAgencyPendingTemplate(data: {
  ...
  return {
    subject: `Nova Imobiliária: ${data.agencyName} - GarantFácil`,
...
export function weeklyCommissionReportTemplate(data: {
  ...
  return {
    subject: `${data.agencyName} - Comissões Pagas: ${data.weekStart} a ${data.weekEnd} - GarantFácil`,
...
export function ticketNotificationTemplate(data: {
  ...
  const description = isNewTicket
    ? 'A GarantFácil abriu um novo chamado para sua imobiliária.'
    : 'A GarantFácil respondeu a um chamado da sua imobiliária.';
  ...
  return {
    subject: `${subjectText} - GarantFácil`,
    html: generateEmailWrapper(content, isNewTicket ? 'A GarantFácil abriu um novo chamado' : 'Você recebeu uma resposta no chamado', IMAGES.office)
  };
}
...
export function invoiceAvailableTemplate(data: {
  ...
  return {
    subject: `${data.agencyName}, fatura disponível - ${data.invoiceMonth}/${data.invoiceYear} - GarantFácil`,
...
export function invoiceDueReminderTemplate(data: {
  ...
  return {
    subject: `${data.agencyName}, fatura vence em ${data.daysUntilDue} dias - GarantFácil`,
...
export function invoiceOverdueTemplate(data: {
  ...
  return {
    subject: `⚠️ ${data.agencyName}, fatura em atraso - ação necessária - GarantFácil`,
...
export function preBlockingAlertTemplate(data: {
  ...
  return {
    subject: `🔒 ${data.agencyName}, bloqueio iminente - regularize a situação agora - GarantFácil`,
...
export function blockingConfirmationTemplate(data: {
  ...
  return {
    subject: `🔒 ${data.agencyName}, conta bloqueada - GarantFácil`,
...
export function boletoUploadedTemplate(data: {
  ...
  return {
    subject: `Boleto disponível - Fatura ${data.invoiceMonth}/${data.invoiceYear} - GarantFácil`,
...
// Template: Nova Garantia Solicitada (para equipe GarantFácil)
export function claimCreatedTemplate(data: {
  ...
  return {
    subject: `Nova garantia solicitada - ${data.tenantName} (${data.agencyName}) - GarantFácil`,
    html: generateEmailWrapper(content, `Nova solicitação de garantia de ${data.agencyName}`)
  };
}

// Template: Alerta de Prazo de Garantia (para equipe GarantFácil)
export function claimDeadlineAlertTemplate(data: {
  ...
  return {
    subject: `${subjectPrefix}: Garantia ${data.claimId.slice(0, 8).toUpperCase()} - ${data.daysRemaining} dias restantes - GarantFácil`,
...
export function claimStatusChangedTemplate(data: {
  ...
  return {
    subject: `Garantia atualizada: ${newStatusLabel} - ${data.tenantName} - GarantFácil`,
